...
def plot_ti(ti_dict:list, ti_colours:list) -> None:
    ...
    * `ti_colours`: List of colours
    ...
    # Plot data
    for ti_list, ti_colour in zip(ti_grid, ti_colours):
        plt.scatter([EVAL_STRAINS[-1]], ti_list[-1], color=ti_colour, s=8**2)
        plt.plot(EVAL_STRAINS, ti_list, color=ti_colour, linewidth=3)

    # Add legend
    handles = [plt.plot([], [], color=ti_colour, linewidth=3, label=header)[0] for header, ti_colour in zip(HEADERS, ti_colours)]
